...
class InvoiceProductReport(models.TransientModel):
    _name = "wizard.inv.product.history.sale.team"
    _description = "Current Invoice Sales History"


    start_date = fields.Date(string='Start Date', required=True)
    end_date = fields.Date(string="End Date", required=True)
    sale_team = fields.Many2one('crm.team',store=True ,compute='_compute_team')


    @api.model
    def default_get(self, default_fields):
        rec = super(InvoiceProductReport, self).default_get(default_fields)
        rec.update({
            'sale_team' : self.sale_team.search([('user_id','=',self.env.user.id)],limit=1).id
    ...
    def export_xls(self):
        ...
        objinvoice = self.env['account.move'].search(
            [('invoice_date', '>=', self.start_date), ('invoice_date', '<=', self.end_date), ('state', '=', 'posted'),
             ('move_type', 'in', ('out_invoice', 'out_refund')),
             ('company_id', '=', self.env.company.id),('team_id','=',self.sale_team.id)])
        ...
        objinvln = self.env['account.move.line'].search(
            [('move_id', 'in', invids), ('product_id', '!=', False), ('account_id', '=', 89)], order='move_id,id')
        ...
        for rec in objinvln:
            ...
            colno += 1
            sheet.write(rowno, colno, str(rec.move_id.team_id.name if rec.move_id.team_id.name else ""),
                        wbf['content_border'])

            colno += 1
            sheet.write(rowno, colno, str(rec.move_id.invoice_user_id.name if rec.move_id.invoice_user_id.name else ""),
                        wbf['content_border'])
            colno += 1
            sheet.write(rowno, colno, str(rec.move_id.partner_id.name if rec.move_id.partner_id.name else ""),
                        wbf['content_border'])
            colno += 1
...
#             colno += 1
            ...
            total_sales = total_sales+rec.price_unit
            ...
            total_sales_price = total_sales_price+int(rec.quantity * multplication)
            total_qty = total_qty+rec.quantity * multplication
            ...
            total_sales_price = total_sales_price+(rec.price_subtotal * multplication)
            # colno += 1
            # sheet.write(rowno, colno, ((rec.price_subtotal * multplication) - (costprice * rec.quantity * multplication)),
        ...
        #             wbf['content_int_border_total'])
        colno = 11
        sheet.write(rowno, colno,str(total_sales),
                    wbf['content_float_border_total'])
        colno += 1

        sheet.write(rowno, colno,str(total_qty),
                    wbf['content_float_border_total'])
        colno += 1
        ...
        sheet.write(rowno, colno,str(total_sales_price),
                    wbf['content_float_border_total'])
        # colno += 1
        # sheet.write(rowno, colno, "=sum(" + chr(65 + colno) + "2:" + chr(65 + colno) + str(rowno) + ")",
        #             wbf['content_float_border_total'])
        ...
        for recpartner in summary_partner:
            ...
            colno = 0
            worksheet4.write(rowno, colno, dic_partners['partnername'] if dic_partners['partnername'] else '',
                             wbf['content_border'])
            colno += 1
